-- ═══════════════════════════════════════════════════════════════
-- VIEW 1: mart_funnel_monthly  (Azure SQL / T-SQL)
-- Dashboard Page: Funnel Leakage
-- Purpose: Step-level conversion and drop-off by signup month
-- ═══════════════════════════════════════════════════════════════
...
step_counts AS (
    SELECT [month], N'Signup' AS step_name, 1 AS step_order,
           COUNT(DISTINCT account_id) AS users_at_step
    FROM cohort GROUP BY [month]

    UNION ALL

    SELECT [month], N'First Invoice Created', 2,
           COUNT(DISTINCT account_id)
    FROM cohort WHERE first_invoice_date IS NOT NULL GROUP BY [month]

    UNION ALL

    SELECT [month], N'First Payment Received', 3,
           COUNT(DISTINCT account_id)
    FROM cohort WHERE first_payment_date IS NOT NULL GROUP BY [month]

    UNION ALL

    SELECT [month], N'App Integration Connected', 4,
           COUNT(DISTINCT account_id)
    FROM cohort WHERE app_integration_date IS NOT NULL GROUP BY [month]

    UNION ALL

    SELECT [month], N'Plan Upgrade', 5,
           COUNT(DISTINCT account_id)
    FROM cohort WHERE plan_upgrade_date IS NOT NULL GROUP BY [month]
),
...
SELECT
    ...
    CASE
        ...
        ELSE ROUND(CAST(users_at_step AS FLOAT) / prev_users, 4)
    END AS conversion_from_prev,
    CASE
        WHEN prev_users IS NULL OR prev_users = 0 THEN 0.0
        ELSE ROUND(1.0 - (CAST(users_at_step AS FLOAT) / prev_users), 4)
